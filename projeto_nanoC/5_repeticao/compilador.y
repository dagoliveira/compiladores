...
DECLARACAO_FUNCAO: TIPO ID {tab_simbolos = insere_simbolo_ts(tab_simbolos, novo_simbolo($2, yylineno, FUNCAO, escopo_atual, $1)); escopo_atual++;}
                 '(' LISTA_ARGS ')' {
                     ...
                     emite_inicio_funcao(out_file, funcao_atual, $5);
...
ENUNCIADO: ID '=' EXPRESSAO_SIMPLES ';' {emite_atribuicao(out_file, $1, $3, tab_simbolos);}
         ...
         | WHILE {emite_inicio_while(out_file);} '(' EXPRESSAO_RELACIONAL ')' '{' {escopo_atual++; emite_fim_teste_while(out_file, $4);} LISTA_ENUNCIADOS '}' {
             imprime_tabela_simbolos(log_file, tab_simbolos); 
             tab_simbolos = remove_simbolos(tab_simbolos, escopo_atual);
             escopo_atual--;
			 emite_fim_while(out_file);
         }
         ;
...
EXPRESSAO_RELACIONAL: EXPRESSAO_SIMPLES IGUAL EXPRESSAO_SIMPLES {$$ = emite_expressao_relacional(out_file, $1, EQ, $3);}
                    | EXPRESSAO_SIMPLES MAIORIGUAL EXPRESSAO_SIMPLES {$$ = emite_expressao_relacional(out_file, $1, GE, $3);}
                    | EXPRESSAO_SIMPLES MENORIGUAL EXPRESSAO_SIMPLES {$$ = emite_expressao_relacional(out_file, $1, LE, $3);}
                    | EXPRESSAO_SIMPLES DIFERENTE EXPRESSAO_SIMPLES {$$ = emite_expressao_relacional(out_file, $1, NE, $3);}
                    | EXPRESSAO_SIMPLES '>' EXPRESSAO_SIMPLES {$$ = emite_expressao_relacional(out_file, $1, GT, $3);}
                    | EXPRESSAO_SIMPLES '<' EXPRESSAO_SIMPLES {$$ = emite_expressao_relacional(out_file, $1, LT, $3);}
                    ;
...
FATOR: ID {
     ...
     | ID '(' LISTA_EXPRESSAO_SIMPLES ')' {
         $$ = emite_chamada_funcao(out_file, busca_simbolo(tab_simbolos, $1), $3);
     }
     | NUM {
