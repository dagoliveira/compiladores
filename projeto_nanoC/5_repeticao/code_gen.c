...
// id_while serve para controlar while, uma forma de identificar unicamente
// um while, usado para controlar os rotulos (labels) de pulo do while
int id_while = 0;

#define STACK_SIZE 100 // 100 whiles aninhados eh demais!!!
int stack[STACK_SIZE]; // pilha para labels do while
...
void emite_inicio_funcao(FILE *fp, struct simbolo *func, struct lista_simbolo *args){
	...
	switch (func->tipo){
		...
		case RELACIONAL: yyerror("funcao nao pode ser do tipo relacional");
	}
...
void emite_atribuicao(FILE *fp, char *id, registrador reg, struct tabela_simbolos *ts){
	if(reg.tipo == VAZIO)
		yyerror("operacao com tipo VOID");
...
registrador emite_exp_simples_int(FILE *fp, registrador reg_l, char operacao, registrador reg_r){
	if(reg_l.tipo == VAZIO || reg_r.tipo == VAZIO)
		yyerror("operacao com tipo VOID");
...
registrador emite_chamada_funcao(FILE *fp, struct simbolo *func, struct lista_reg *args){
	...
	switch (func->tipo){
		...
		case REAL: sprintf(tipo,"float"); break;
		case VAZIO: sprintf(tipo,"void"); break;
		case RELACIONAL: yyerror("operador relacional nao eh permitido como argumento de funcao");
	}
...
registrador emite_expressao_relacional(FILE *fp, registrador reg_l, OpRelacional operacao, registrador reg_r){
	if(reg_l.tipo == VAZIO || reg_r.tipo == VAZIO)
		yyerror("operacao com tipo VOID");
	char comp_instrucao[10] = "icmp";
	char tipo[10] = "i32";
	char comparacao[10];
	if (reg_l.tipo != reg_r.tipo){ // promover int para float
		strncpy(comp_instrucao, "fcmp",10);
		strncpy(tipo, "float",10);
		if(reg_l.tipo == INTEIRO)
			reg_l = cast_integer_float(fp, reg_l);
		else
			reg_r = cast_integer_float(fp, reg_r);
	}
	registrador reg = novo_registrador(RELACIONAL);
	int inteiro = (reg_l.tipo == INTEIRO);
	switch (operacao){
		case EQ: 
			if (inteiro)
				strncpy(comparacao, "eq", 10);
			else
				strncpy(comparacao, "oeq", 10);
			break;
		case NE:
			if (inteiro)
				strncpy(comparacao, "ne", 10);
			else
				strncpy(comparacao, "one", 10);
			break;
		case GE:
			if (inteiro)
				strncpy(comparacao, "sge", 10);
			else
				strncpy(comparacao, "oge", 10);
			break;
		case LE:
			if (inteiro)
				strncpy(comparacao, "sle", 10);
			else
				strncpy(comparacao, "ole", 10);
			break;
		case GT:
			if (inteiro)
				strncpy(comparacao, "sgt", 10);
			else
				strncpy(comparacao, "ogt", 10);
			break;
		case LT:
			if (inteiro)
				strncpy(comparacao, "slt", 10);
			else
				strncpy(comparacao, "olt", 10);
			break;
	}
    fprintf(fp, "\t%s = %s %s %s %s, %s\n",reg.nome, comp_instrucao, comparacao, tipo, reg_l.nome, reg_r.nome);
...
void emite_inicio_while(FILE *fp){
    id_while++;
	push(id_while);
    fprintf(fp, "\tbr label %%while.%d\n", id_while); // Precisa fechar um  bloco basico antes de iniciar outro
    fprintf(fp, "while.%d:\n", id_while); // iniciando um bloco basico, que sera para o teste do while
}
...
void emite_fim_teste_while(FILE *fp, registrador reg){
	int while_stacked = peek();
    if(reg.tipo != RELACIONAL){
        yyerror("Expressao nao relacional, booleana, no if");
        return;
    }
    fprintf(fp, "\tbr i1 %s, label %%dowhile.%d, label %%endwhile.%d\n",reg.nome, while_stacked, while_stacked);
    fprintf(fp, "dowhile.%d:\n", while_stacked);
}
